...
df = pd.read_csv('out.csv',index_col=["SAMPN","PERNO"])
...
df.XCORD += np.random.random(len(df.index))*.001
df.YCORD += np.random.random(len(df.index))*.001
df = df[['INCOM','AGE','AREA','MODE','ARR_HR','XCORD','YCORD']]
df.MODE = df.MODE.fillna(6)
df = df[df.INCOM<=10]
df.INCOM[df.INCOM==2] = 3
df.INCOM[df.INCOM==1] = 2
df.INCOM[df.INCOM==10] = 9
df.INCOM = df.INCOM-1
df = df[df.AGE<=100]
print df.INCOM.value_counts()
#print df.describe()
...
for region in regions.items():
  ...
  for name, group in tmpdf.groupby('INCOM'):
    del group["INCOM"]  
    group.to_csv('%s/minibats%s.csv'%(region[1],name),index_label=['HHID','PERID'],float_format="%.3f")
